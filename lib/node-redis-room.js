...
module.exports = (function () {
  ...
  function init(io, crud, sub, pub, callback) {
    debug('init');
    _io = io,
    _crud = crud; 
    _sub = sub;
    ...
    _sub.on('message', function (channel, message) {
      ...
      _io.in(channel).emit(message.cmd, message.content);
    });
    
    _sub.on('subscribe', function (channel, count) {
      debug('sub subscribe %s %s', channel, count);
    });
  }
  ...
  function join(socket, roomName, userObj, callback) {
    ...
    async.auto({
      ...
      socket_join: ['validate', function (cb) {
        socket.join(roomName, function (err) {
          if (err) {
            cb(err);
          } else {
            debug("join socket.join success");
            cb();
          }
        });
      }],  
      join_room: ['socket_join', function (cb) {
  ...
  function leave(socket, roomName, userObj, callback) {
      ...
      socket_leave: function (cb) {
        socket.leave(roomName, function (err) {
          if (err) {
            ...
            debug("leave socket.leave success");
            cb();
          }
        });
      },  
      leave_room: ['socket_leave', function (cb) {
